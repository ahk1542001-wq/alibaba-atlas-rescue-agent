...
Safe fields only — the existing 8 (`passport_country`, `passport_no` (masked), `expiry`, `home_city`, `cabin`, `diet`, `budget_range`, `airlines_like`). Each field: value (masked passport renders server-masked value `MD*****67` style — never unmasked), **Edit / Delete** actions at ≥44px. **Persistence consent:** the existing consent gate (`profile-consent`) becomes the first visible element with plain text: "Only save details I give you. You can delete anything at any time." No consent → fields are session-only, clearly labelled. **Explicit statement, always visible at the drawer footer:** "We never store your full passport number, payment details, or legal identity unmasked. Passport numbers are shown and stored masked (e.g. MD\*\*\*\*\*67)." — aligned with `profile.py` boundary masking. Profile-field PUTs keep `source:'user'`; inferred values show a "Suggested by Atlas" tag with one-tap confirm/reject.
